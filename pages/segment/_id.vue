...
export default {
  ...
  async asyncData ({ $axios, $luxon, params }) {
    const segment = (await $axios.$get('https://bokoblin.herokuapp.com/?query={segment(id:' + params.id + '){id,game{title,id,isZelda,isEvent},modifier,runners{name,id},filenames{filename,note},raised,start_time,end_time,vod,time_offset,marathon{id,color,full_name,slug,type_id,total,start_date,playlist,charity{id,full_name}}}}')).data.segment
